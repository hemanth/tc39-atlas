...
An interactive web application applying the **TypeSafe AI** System One paradigm (`Jev`) to the live corpus of [ECMAScript / TC39 proposals](https://tc39.es/).

Built with strict frontend design taste (`tasteskill.dev`), featuring a calibrated palette, deterministic typography, zero emojis, and an asymmetric split-screen architecture.

---

## Design & Aesthetic (Taste Skill Guidelines)
...
- **Typography**: Clean modern sans-serif (`Plus Jakarta Sans` / `Geist`) paired with high-clarity monospace (`JetBrains Mono`) for probabilities, ratings, and code tokens, with editorial serif display accents.
...
While standard records provide basic committee metadata (`stage`, `name`, `description`, `authors`), they lack structural and contextual nuance. TypeSafe AI enriches every proposal with a multi-dimensional semantic taxonomy evaluated using System One (`Jev`):
...
- **Pipeline**:
  1. Pulls the latest official TC39 committee archive.
